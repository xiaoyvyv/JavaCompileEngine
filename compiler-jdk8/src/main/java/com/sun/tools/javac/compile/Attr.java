    ...
    Type check(final JCTree tree, final Type found, final int ownkind, final ResultInfo resultInfo) {
        ...
        if (!found.hasTag(ERROR) && !resultInfo.pt.hasTag(METHOD) && !resultInfo.pt.hasTag(FORALL)) {
            if ((ownkind & ~resultInfo.pkind) != 0) {
                ...
                inferenceContext.addFreeTypeListener(List.of(found, resultInfo.pt), new Infer.FreeTypeListener() {
                    @Override
 ...
 *************************************************************************/
        ...
        try {
            ...
            try {
                ...
                for (JCTree resource : tree.resources) {
                    Check.CheckContext twrContext = new Check.NestedCheckContext(resultInfo.checkContext) {
                        @Override
                        public void report(DiagnosticPosition pos, JCDiagnostic details) {
    ...
    //where
        ...
        private void checkAccessibleTypes(final DiagnosticPosition pos, final Env<AttrContext> env,
                                          final Infer.InferenceContext inferenceContext, final Type... ts) {
            checkAccessibleTypes(pos, env, inferenceContext, List.from(ts));
        }

        private void checkAccessibleTypes(final DiagnosticPosition pos, final Env<AttrContext> env,
                                          final Infer.InferenceContext inferenceContext, final List<Type> ts) {
            if (inferenceContext.free(ts)) {
                inferenceContext.addFreeTypeListener(ts, new Infer.FreeTypeListener() {
                    @Override
                    public void typesInferred(Infer.InferenceContext inferenceContext) {
        ...
        class ExpressionLambdaReturnContext extends FunctionalReturnContext {
            ...
            ExpressionLambdaReturnContext(JCExpression expr, Check.CheckContext enclosingContext) {
                super(enclosingContext);
                this.expr = expr;
            }
        ...
        */
        private void checkLambdaCompatible(JCLambda tree, Type descriptor, Check.CheckContext checkContext) {
    ...
    void checkReferenceCompatible(JCMemberReference tree, Type descriptor, Type refType, Check.CheckContext checkContext, boolean speculativeAttr) {
        Type returnType = checkContext.inferenceContext().asUndetVar(descriptor.getReturnType());
    ...
    private void setFunctionalInfo(final Env<AttrContext> env, final JCFunctionalExpression fExpr,
        ...
        if (checkContext.inferenceContext().free(descriptorType)) {
            checkContext.inferenceContext().addFreeTypeListener(List.of(pt, descriptorType), new Infer.FreeTypeListener() {
                public void typesInferred(Infer.InferenceContext inferenceContext) {
    ...
    //where
        ...
        Type checkMethodIdInternal(JCTree tree,
            ...
            if ((resultInfo.pkind & POLY) != 0) {
                Type pt = resultInfo.pt.map(deferredAttr.new RecoveryDeferredTypeMap(DeferredAttr.AttrMode.SPECULATIVE, sym, env.info.pendingResolutionPhase));
                Type owntype = checkIdInternal(tree, site, sym, pt, env, resultInfo);
                resultInfo.pt.map(deferredAttr.new RecoveryDeferredTypeMap(DeferredAttr.AttrMode.CHECK, sym, env.info.pendingResolutionPhase));
                return owntype;
            } else {
...
//                             Resolve.isStatic(env));//DEBUG
    ...
    public Type checkMethod(Type site,
        ...
        try {
            ...
            List<Type> argtypes2 = Type.map(argtypes,
                    rs.new ResolveDeferredRecoveryMap(DeferredAttr.AttrMode.CHECK, sym, env.info.pendingResolutionPhase));
    ...
    private final class TypeAnnotationsValidator extends TreeScanner {
         ...
         * Check.Validator.visitSelectInternal(JCFieldAccess)
